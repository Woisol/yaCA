...
export function Rewind({ sessions, onSessionSelect, onQuit }: { sessions: SessionMeta[]; onSessionSelect(sessionId: string): void; onQuit(): void }) {
    ...
    <Box flexDirection="column">
      {sessions.length === 0 ? (
        ...
        sessions.map((session, index) => (
          <Box key={session.id} backgroundColor={index === selectedIndex ? "cyanBright" : undefined}>
