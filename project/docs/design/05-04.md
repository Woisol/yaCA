...
### 5. 工具集规划（第一版）
 ...
 | `read_file`      | 读取文件内容       | `path`, `startLineNumber?`, `startColumn? = 0`, `endLineNumber?`, `endColumn? = 0`             |
 | `write_file`     | 创建或完全覆写文件 | `path`, `content`, `append?`, `encoding?`, `dangerouslyOverride?`                      |
 | `replace_file`   | 精准替换文件内容   | `path`, `new_text`, `startLineNumber?`, `startColumn? = 0`, `endLineNumber?`, `endColumn? = 0` |
...
### 5.1. 工具集规划（第二版）
 ...
 | `cwd`  | 获取当前工作目录   |                                                                             |
 | `get_tool_hint`  | 获取工具使用格式   | `toolName?`                                                                            |
 | `stat_path`      | 检查路径状态       | `path`              |
 | `read_file`      | 读取文件内容       | `path`, `startLineNumber?`, `startColumn? = 0`, `endLineNumber?`, `endColumn? = 0`             |
 | `write_file`     | 创建或完全覆写文件 | `path`, `content`, `append?`, `encoding?`, `dangerouslyOverride?`                      |
 | `replace_file`   | 精准替换文件内容   | `path`, `new_text`, `startLineNumber?`, `startColumn? = 0`, `endLineNumber?`, `endColumn? = 0` |
 | `list_directory` | 列出目录结构       | `path`, `recursive?`                                                                   |
 | `move_file` | 移动文件/目录       | `source`, `destination`, `dangerouslyOverride?`                                                                   |
 | `remove_file` | 删除文件/目录       | `path`, `recursive?`, `dangerouslyRemoveOutsideOfWorkspace?`                                                                   |
 | `search_files`   | 全文搜索           | `pattern`, `path?`, `include?`                                                         |
 | `exec_command`   | 执行 Shell 命令    | `command`, `timeout?`, `cwd?`                                                          |
