...
项目选定使用 Qwen2.5-VL 作为首选多模态模型，使用 Ollama 进行本地部署。
...
| Ctrl+C      | 中断当前操作  | 如果Agent正在生成，则停止生成          |
...
| 命令           | 功能         | 说明                                          |
| -------------- | ------------ | --------------------------------------------- |
| /model <name>  | 切换模型     | 支持切换到不同的LLM（如Qwen2.5-VL、Claude等） |
...
```json
{
  ...
  "models": [
    {"name": "qwen2.5-vl-7b", "base_url": "https://api.qwen.ai/v1"},
